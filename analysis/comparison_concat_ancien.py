"""
Comparison analysis module for AdaptFoundation project - CONCATENATION STRATEGY.

This module implements comprehensive analysis of concatenation-based classification results
...
class ComparisonAnalyzerConcat:
    """
    Analyzer for comparing concatenation-based classification results across different configurations.
    
    This class handles loading and parsing of classification results from
    multiple foundation models, concatenation configurations, and PCA strategies.
    
    Attributes:
        features_base_path (Path): Base path to feature_extracted_concat directory
    """
    
    def __init__(self, features_base_path: str):
        """
        Initialize the concatenation comparison analyzer.
        
        Args:
            features_base_path (str): Path to feature_extracted_concat directory
        """
    ...
    def collect_all_results(self) -> pd.DataFrame:
        ...
        if not df.empty:
            print(f"Collected {len(df)} concatenation experimental results")
            print(f"Models: {df['model'].nunique()}")
            print(f"Concatenation configurations: {df['config'].nunique()}")
    ...
    def filter_logistic_only(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Filter dataset to keep only logistic regression results for concatenation.
        
        Args:
            df (pd.DataFrame): Complete concatenation results dataset
        ...
        print(f"Filtered to {len(logistic_df)} logistic regression concatenation results")
        
        return logistic_df
    
    def get_top_10_configurations(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Get top 10 concatenation configurations ranked by test ROC-AUC performance.
    ...
    def create_top10_table(self, df: pd.DataFrame) -> plt.Figure:
        ...
        # Create table data with concatenation-specific formatting
        table_data = []
        for i, row in df_table.iterrows():
            ...
            # Add feature dimension (important for concatenation)
        ...
        # Create figure with adjusted size for additional column
        fig, ax = plt.subplots(figsize=(18, 8))
        ax.axis('off')
        
        # Define column headers with feature dimension
        ...
        for i in range(1, len(table_data) + 1):
            row_color = '#FFF3E0' if i % 2 == 1 else '#FFE0B2'  # Orange theme for concatenation
