#!/usr/bin/env python3
"""
Classification pipeline for SAM-Med3D density optimization approaches with CV investigation.
...
Usage:
python sam3d_variantes/turbo_with_density/run_pipeline_classification_density_investigation.py
python sam3d_variantes/turbo_with_density/run_pipeline_classification_density_investigation.py --config classification_density_investigation.yaml
"""
...
try:
    ...
    print("Classification pipeline with CV investigation initialized for SAM-Med3D density approaches")
...
class DensityLinearProberWithInvestigation:
    """
    Linear probing classifier for SAM-Med3D density approaches with CV investigation.
    
    Specialized for density-optimized features with detailed fold-level analysis
    to investigate Test vs CV score discrepancy phenomenon.
    """
    
    def __init__(self, input_base_path: str, output_base_path: str, model_name: str = 'sam_med3d_turbo_density',
                 random_state: int = 42, n_jobs: int = -1, 
                 classifier_params: Dict = None, investigation_config: Dict = None):
        """
        Initialize the density linear probing classifier with investigation capabilities.
        
        Args:
            input_base_path (str): Base path to read features from (feature_extraction_density)
            output_base_path (str): Base path to save results to (sam3d_variantes/turbo_with_density)
        ...
        # Validate paths
        self.input_model_path = self.input_base_path / model_name
        if not self.input_model_path.exists():
            raise FileNotFoundError(f"Input directory not found: {self.input_model_path}")
        
        # Create output directory if needed
        self.output_base_path.mkdir(parents=True, exist_ok=True)
        
        print(f"DensityLinearProberWithInvestigation initialized:")
        print(f"  Input path: {self.input_model_path}")
    ...
    def _load_all_cv_data(self, approach_name: str, pca_mode: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        Args:
            ...
            pca_mode (int): PCA mode (32, 256, 95, 99)
        
        Returns:
            Tuple[np.ndarray, np.ndarray, np.ndarray]: Features (X), labels (y), groups
        """
        approach_path = self.input_model_path / approach_name / f"PCA_{pca_mode}"
    ...
    def _extract_cv_detailed_analysis(self, cv_results_df: pd.DataFrame, 
                                     ...
                                     cv_score: float, approach_name: str) -> Dict:
        """
        ...
        Returns:
            Dict: Detailed CV analysis including fold scores and density-specific diagnostics
        """
        fold_test_scores = []
        fold_train_scores = []
        ...
        density_stability_score = 1.0 - np.std(fold_test_scores)  # Higher = more stable
        is_masking_approach = 'masking' in approach_name
        is_weighting_approach = 'weighting' in approach_name
        ...
        cv_detailed_analysis = {
            ...
            'density_approach_analysis': {
                'approach_name': approach_name,
                'stability_score': density_stability_score,
                'is_masking_approach': is_masking_approach,
                'is_weighting_approach': is_weighting_approach,
                'approach_impact_on_stability': 'improved' if density_stability_score > 0.95 else 'neutral' if density_stability_score > 0.90 else 'degraded'
            }
        }
        
        return cv_detailed_analysis
    
    def _save_cv_results_detailed(self, approach_name: str, pca_mode: int, 
        ...
        Args:
            ...
            pca_mode (int): PCA mode identifier
        ...
        output_dir = self.output_base_path / approach_name / f"PCA_{pca_mode}"
        ...
        # Investigation metadata with density-specific information
        investigation_metadata = {
            ...
            'density_approach_info': {
                'approach_name': approach_name,
                'is_baseline': 'baseline' in approach_name,
                'is_masking': 'masking' in approach_name,
                'is_weighting': 'weighting' in approach_name,
                'variable_dimensions': 'masking' in approach_name
            }
        }
    ...
    def train_classifier(self, approach_name: str, pca_mode: int) -> Dict:
        ...
        self._save_cv_results_detailed(approach_name, pca_mode, cv_results_df, clf)
        ...
        cv_detailed_analysis = self._extract_cv_detailed_analysis(
            cv_results_df, best_idx, test_roc_auc_weighted, clf.best_score_, approach_name
        )
    ...
    def get_available_approaches(self) -> List[str]:
        ...
        """
        approaches = []
        for path in self.input_model_path.iterdir():
    ...
    def check_pca_availability(self, approach_name: str, pca_mode: int) -> bool:
        ...
        """
        pca_path = self.input_model_path / approach_name / f"PCA_{pca_mode}"
        required_files = [
...
def get_density_classification_plan(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    print(f"Generated {len(tasks)} density classification tasks with investigation")
    print(f"Models: {len(models)}, Approaches: {len(configurations)}, Classifiers: {len(classifiers)}, PCA modes: {len(pca_modes)}")
    
    return tasks


def validate_density_task(task: Dict[str, Any], input_base_path: str, 
    ...
    Args:
        ...
        input_base_path (str): Path to feature_extraction_density/sam_med3d_turbo_density directory
    ...
    approach_path = Path(input_base_path) / task['model'] / task['approach']
...
def execute_density_task_with_investigation(task: Dict[str, Any], input_base_path: str, 
                                          output_base_path: str, reporting_config: Dict[str, Any], 
                                          yaml_config: Dict[str, Any]) -> Dict[str, Any]:
    """
    Execute a single density classification task with investigation.
    
    Args:
        task (Dict[str, Any]): Task to execute  
        input_base_path (str): Path to feature_extraction_density directory
        output_base_path (str): Path to sam3d_variantes/turbo_with_density directory
    ...
    if reporting_config.get('verbose', True):
        print(f"Executing: {approach} | {classifier} | PCA_{pca_mode}")
    ...
    try:
        ...
        prober = DensityLinearProberWithInvestigation(
...
def save_density_results_with_investigation(task: Dict[str, Any], result_data: Dict[str, Any], 
                                           output_base_path: str, output_config: Dict[str, Any]) -> None:
    """
    Save density classification results with investigation analysis.
    ...
    if output_config.get('save_individual', True):
        approach_dir = Path(output_base_path) / approach / f"PCA_{pca_mode}"
    ...
    if output_config.get('save_consolidated', True):
        consolidated_file = Path(output_base_path) / "classification_results_density_sam_med3d_turbo_density.json"
        ...
        if consolidated_file.exists():
            ...
            all_results = {
                'experiment_info': {
                    'pipeline': 'SAM-Med3D Density Classification with CV Investigation',
        ...
        if f"PCA_{pca_mode}" not in all_results['results'][approach]:
            all_results['results'][approach][f"PCA_{pca_mode}"] = {}
        all_results['results'][approach][f"PCA_{pca_mode}"][classifier] = result
        
        # Update investigation analysis
        _update_investigation_summary(all_results, approach, pca_mode, result)
        
        with open(consolidated_file, 'w') as f:
            json.dump(all_results, f, indent=2, default=str)


def _update_investigation_summary(all_results: Dict, approach: str, pca_mode: int, result: Dict) -> None:
    """
    ...
    config_key = f"{approach}_PCA_{pca_mode}"
    ...
    if 'cv_detailed_analysis' in result:
        ...
        all_results['investigation_analysis'][config_key] = {
            ...
            'density_stability_score': cv_analysis['density_approach_analysis']['stability_score'],
            'approach_type': cv_analysis['density_approach_analysis']['approach_name']
        }


def run_yaml_density_classification_with_investigation(config_file: str) -> None:
    """
    Run density classification pipeline with CV investigation using YAML configuration.
    
    Args:
        config_file (str): Path to YAML configuration file
    """
    print("=" * 80)
    print("SAM-MED3D DENSITY CLASSIFICATION WITH CV INVESTIGATION")
    print("=" * 80)
    print(f"Configuration: {config_file}")
    
    config = load_yaml_config(config_file)
    input_base_path = "feature_extraction_density"  # Read features from here
    output_base_path = config['output']['base_path']  # Save results to YAML config path
    ...
    valid_tasks = []
    for task in tasks:
        if validate_density_task(task, input_base_path, config['validation']):
            valid_tasks.append(task)
    
    print(f"Execution Plan: {len(valid_tasks)}/{len(tasks)} valid tasks")
    print(f"Expected phenomenon: Test ROC-AUC (~75%) > CV ROC-AUC (~67%) - more marked than DINOv2")
    ...
    for i, task in enumerate(valid_tasks, 1):
        ...
        result_data = execute_density_task_with_investigation(task, input_base_path, output_base_path, config['reporting'], config)
        results.append({'task': task, 'result': result_data})
        
        save_density_results_with_investigation(task, result_data, output_base_path, config['output'])
    ...
    print("\n" + "=" * 80)
    print("DENSITY CLASSIFICATION WITH INVESTIGATION COMPLETED")
    print("=" * 80)
    ...
    if successful > 0:
        print(f"\nResults structure:")
        print(f"sam3d_variantes/turbo_with_density/")
        ...
        print(f"sam3d_variantes/turbo_with_density/classification_results_density_sam_med3d_turbo_density.json")
        ...
        for r in results:
            if r['result']['status'] == 'success':
                ...
                gaps.append(gap)
                print(f"  {task['approach']}/PCA_{task['pca_mode']}: CV={cv_score:.4f} | Test={test_score:.4f} | Gap={gap:.4f} | ProbFolds={prob_folds}")
        ...
        print(f"\nNext steps:")
        print(f"1. Analyze fold-level patterns across density approaches")
        print(f"2. Compare baseline vs masking vs linear_weighting stability")
        print(f"3. Investigate if masking approach affects fold consistency")
        print(f"4. Evaluate density weighting impact on CV stability")
        print(f"5. Compare SAM-Med3D vs DINOv2 investigation patterns")
...
def run_single_density_approach_with_investigation(input_base_path: str, output_base_path: str, 
                                                  model_name: str, approach_name: str, pca_mode: int,
                                                  classifier_params: Dict = None, 
                                                  investigation_config: Dict = None) -> None:
    """
    Run classification for a single density approach with investigation.
    ...
    """
    print("=" * 80)
    print("SAM-MED3D DENSITY CLASSIFICATION - SINGLE APPROACH WITH INVESTIGATION")
    print("=" * 80)
    print(f"Input base path (reading): {input_base_path}")
    print(f"Output base path (writing): {output_base_path}")
    ...
    try:
        ...
        # Save results
        approach_dir = Path(output_base_path) / approach_name / f"PCA_{pca_mode}"
        approach_dir.mkdir(parents=True, exist_ok=True)
        results_file = approach_dir / "classification_results.json"
...
def main():
    """
    Main entry point for density classification with investigation script.
    """
    parser = argparse.ArgumentParser(
        description="Run linear probing classification on SAM-Med3D density-optimized features with CV investigation",
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="classification_density_investigation.yaml",
    ...
    parser.add_argument(
        ...
        help="Path to feature_extraction_density directory (for reading features)"
    )
    
    parser.add_argument(
        "--output-base-path",
        type=str,
        default="sam3d_variantes/turbo_with_density",
        help="Path to sam3d_variantes/turbo_with_density directory (for saving results)"
    )
    ...
    if args.approach:
        ...
        print("=== Single Density Approach Classification with Investigation ===")
        ...
        run_single_density_approach_with_investigation(
            str(input_path),
            str(output_path),
            args.model_name,
            args.approach,
        ...
        try:
            run_yaml_density_classification_with_investigation(str(config_path))
        except Exception as e:
