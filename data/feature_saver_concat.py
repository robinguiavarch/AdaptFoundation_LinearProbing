"""
Feature dataset saver with concatenation strategy for storing extracted features with associated metadata.

This module handles the systematic extraction and storage of features from
3D skeletal volumes using concatenation-based pipeline configurations.
"""
...
from pipelines.feature_extraction_pipeline_concat import FeatureExtractionPipeline


class FeatureDatasetSaver:
    """
    Manages extraction and storage of feature datasets from 3D skeletal volumes using concatenation.
    
    This class orchestrates the complete process of running feature extraction
    pipelines with concatenation configurations and saving the results in an
    ...
    def __init__(self, data_path: str, output_base_path: str, model_name: str = 'dinov2_vits14'):
        """
        Initialize the feature dataset saver with concatenation strategy.
    ...
    def save_configuration(self, pooling_strategy: str, 
        ...
        """
        Extract and save features for a specific configuration using concatenation.
        
        Args:
            pooling_strategy (str): Kept for compatibility (unused in concatenation)
    ...
    def save_all_standard_configurations(self) -> None:
        """
        Extract and save features for all standard concatenation configurations.
        
        Processes the following configurations:
        - Multi-axes with concatenation
        - Single axis (axial) with concatenation
        - Single axis (coronal) with concatenation
        - Single axis (sagittal) with concatenation
        """
        configurations = [
            # Multi-axes configuration with concatenation
            ('average', None),
            # Single-axis configurations with concatenation
            ('average', ['axial']),
            ('average', ['coronal']),
            ('average', ['sagittal'])
        ]
        
        print(f"Processing {len(configurations)} standard concatenation configurations...")
