"""
Data loading utilities for HCP LARGE_CINGULATE dataset.
"""
...
class LCDataLoader:
    """
    Data loader for HCP LARGE_CINGULATE skeletal dataset (binary classification).
    ...
    def load_split(self, split_name):
        ...
        for i, subject in enumerate(self.subjects_df['Subject']):
            if str(subject) in split_subjects:
                label_row = self.labels_df[self.labels_df['long_name'] == subject]
                if not label_row.empty:
                    subject_indices.append(i)
                    right_pcs_value = label_row['Right_PCS'].iloc[0]
                    numeric_label = label_mapping[right_pcs_value]
                    split_labels.append(numeric_label)
...
def load_lc_dataset(data_path):
    """
    Load LC dataset with all splits.
    
    Args:
        data_path (str or Path): Path to dataset directory
        
    Returns:
        LCDataLoader: Initialized data loader
    """
    return LCDataLoader(data_path)