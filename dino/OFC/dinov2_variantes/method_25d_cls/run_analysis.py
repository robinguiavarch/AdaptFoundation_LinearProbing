"""
Analysis script for Method 2.5D CLS Token variants classification results.

This script analyzes the 18 combinations from method_25d_cls_features directory
...
class Method25DCLSAnalyzer:
    """
    Analyzer for Method 2.5D CLS Token classification results.
    ...
    def __init__(self, features_base_path: str):
        """
        Initialize the Method 2.5D CLS analyzer.
    ...
    def collect_all_results(self) -> pd.DataFrame:
        ...
        variants = [
            'concat_baseline',
            'concat_25d_no_overlap',
            'concat_25d_overlap',
            'pooling_baseline',
            'pooling_25d_no_overlap',
            'pooling_25d_overlap'
        ]
                ...
                if result_file.exists():
                    try:
                        ...
                        if 'baseline' in variant:
                            method_25d = 'baseline'
                        elif 'no_overlap' in variant:
                            method_25d = '25d_no_overlap'
                        elif 'overlap' in variant:
                            method_25d = '25d_overlap'
                        else:
                            method_25d = 'unknown'
                        ...
                        parsed_result = {
                            'model': 'dinov2_vitg14',
                            'variant': variant,
                            'config': f"{aggregation_method}_{method_25d}",
                            'aggregation_method': aggregation_method,
                            'method_25d': method_25d,
                            'pca_mode': pca_mode,
                            'classifier': 'logistic',
    ...
    def create_aggregation_table(self, df_filtered: pd.DataFrame, aggregation_method: str, 
        ...
        if df_filtered.empty:
            ...
            ax.set_title(f'{aggregation_method.title()} 2.5D CLS - Logistic Regression ({metric_type.upper()})')
            ax.axis('off')
            return fig
        ...
        for i, row in df_sorted.iterrows():
            method_display = row['method_25d'].replace('_', ' ').replace('25d', '2.5D').title()
        ...
        columns = ['Rank', 'Model', 'Method 2.5D', 'PCA', 'Feature Dim', 'Overfitting Gap', 'CV ROC-AUC', 'Test ROC-AUC']
        ...
        ax.set_title(f'{aggregation_method.title()} 2.5D CLS - Logistic Regression ({n_configs} Configurations)\n'
                    f'PCA Modes: {pca_modes_str} - Ranked by {metric_display}', 
                    fontsize=16, fontweight='bold', pad=30)
        
        fig.text(0.5, 0.02, f'DINOv2 Giant CLS tokens (1536D) with 2.5D variants - {aggregation_method.title()} aggregation - Ranked by {metric_display}', 
    ...
    def run_analysis(self) -> Tuple[pd.DataFrame, List[plt.Figure]]:
        """
        Execute complete Method 2.5D CLS analysis workflow.
...
def main():
    """
    Main entry point for Method 2.5D CLS analysis script.
    """
    parser = argparse.ArgumentParser(
        description="Run analysis of Method 2.5D CLS classification results"
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="analysis_results_25d_cls",
    ...
    print("Method 2.5D CLS Analysis")
    print(f"Features path: {features_path}")
    print(f"Output directory: {output_dir}")
    
    analyzer = Method25DCLSAnalyzer(str(features_path))
    
    print("\nExecuting Method 2.5D CLS analysis workflow...")
    df_all, figures = analyzer.run_analysis()
    
    if not df_all.empty:
        output_file_logistic = output_dir / "analysis_results_25d_cls_logistic.csv"
        df_all.to_csv(output_file_logistic, index=False)
        print(f"Saved results: {output_file_logistic}")
    
    figure_names = [
        "concat_25d_cls_table_test",
        "concat_25d_cls_table_cv", 
        "pooling_25d_cls_table_test",
        "pooling_25d_cls_table_cv"
    ]
    ...
    if not df_all.empty:
        print(f"\nMethod 2.5D CLS Analysis Summary:")
    ...
    print(f"\nMethod 2.5D CLS analysis completed successfully!")
