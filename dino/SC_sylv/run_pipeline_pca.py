"""
Pipeline 2: PCA reduction for SC_sylv feature maps features.

This script applies PCA reduction to pre-extracted raw feature maps features
from Pipeline 1, generating final reduced features for regression.
"""
...
from dino.SC_sylv.feature_extraction_core import StandalonePCAProcessor


class SCPCAPipeline:
    """
    Pipeline 2: PCA reduction for pre-extracted SC_sylv feature maps features.
    
    This pipeline loads raw features from Pipeline 1 and applies PCA reduction
    to generate final features ready for regression.
    ...
    def __init__(self, config_path: str):
        """
        Initialize SC_sylv PCA pipeline.
        ...
        print(f"SC_sylv PCA pipeline initialized:")
    ...
    def _save_pca_results(self, result: dict, output_dir: Path, variant_name: str, pca_time: float) -> None:
        ...
        final_metadata = {
            ...
            'task_info': {
                ...
                'n_targets': 6,
                'target_names': ['dim1', 'dim2', 'dim3', 'dim4', 'dim5', 'dim6']
            }
        }
    ...
    def run_pca_reduction(self) -> None:
        ...
        print("=" * 80)
        print("PIPELINE 2: SC_SYLV PCA REDUCTION")
        print("=" * 80)
        ...
        print("PCA-reduced features ready for regression")
        print(f"{'='*80}")


def main():
    """
    Main entry point for SC_sylv PCA reduction pipeline.
    """
    parser = argparse.ArgumentParser(
        description="SC_sylv PCA reduction - Pipeline 2",
    ...
    try:
        pipeline = SCPCAPipeline(str(config_path))
