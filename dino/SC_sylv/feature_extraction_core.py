...
class Method25D:
    ...
    def create_25d_slices_adaptive(self, volume_3d: np.ndarray, axis: str) -> torch.Tensor:
        """
        Create 2.5D slices with adaptive overlapping grouping for SC_sylv dimensions.
        
        Args:
            volume_3d (np.ndarray): Input volume with shape (38, 36, 49)
            axis (str): Slicing axis ('axial', 'coronal', 'sagittal')
        
        Returns:
            torch.Tensor: Processed slices with overlapping step=2
        """
        if axis == 'sagittal':  # X axis - 38 slices (0-37)
            # Create overlapping groups: [0,1,2], [2,3,4], [4,5,6], ..., [34,35,36]
            # Max start_idx = 34 (pour avoir [34,35,36])
            slices_2d = []
            for start_idx in range(0, 36, 2):  # 0, 2, 4, ..., 34 → 18 groups
        ...
        elif axis == 'coronal':  # Y axis - 36 slices (0-35)
            # Create overlapping groups: [0,1,2], [2,3,4], [4,5,6], ..., [32,33,34]
            # Max start_idx = 32 (pour avoir [32,33,34])
            slices_2d = []
            for start_idx in range(0, 34, 2):  # 0, 2, 4, ..., 32 → 17 groups
                group_slices = np.stack([
                    volume_3d[:, start_idx, :],
                    volume_3d[:, start_idx + 1, :], 
                    volume_3d[:, start_idx + 2, :]
        ...
        elif axis == 'axial':  # Z axis - 49 slices (0-48)
            # Create overlapping groups: [0,1,2], [2,3,4], [4,5,6], ..., [46,47,48]
            # Max start_idx = 46 (pour avoir [46,47,48])
            slices_2d = []
            for start_idx in range(0, 47, 2):  # 0, 2, 4, ..., 46 → 24 groups
                group_slices = np.stack([
                    volume_3d[:, :, start_idx],
                    volume_3d[:, :, start_idx + 1], 
                    volume_3d[:, :, start_idx + 2]
    ...
    def create_standard_slices(self, volume_3d: np.ndarray, axis: str) -> torch.Tensor:
        ...
        if axis == 'sagittal':
            ...
            slices_2d = [volume_3d[:, i, :] for i in range(36)]
        elif axis == 'axial':
            slices_2d = [volume_3d[:, :, i] for i in range(49)]
        else:
            raise ValueError(f"Unknown axis: {axis}")
