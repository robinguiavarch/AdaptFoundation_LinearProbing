"""
Test script for SC_sylv feature maps extraction pipeline with separated PCA.

This script validates all components of the SC_sylv feature extraction approach
...
from dino.SC_sylv.feature_extraction_core import (
...
class SCFeatureExtractionTester:
    """
    Tests all components of the SC_sylv feature extraction pipeline.
    ...
    def __init__(self, data_path: str = "../../crops/2mm/S.C.-sylv./"):
        ...
        Args:
            data_path (str): Path to SC_sylv dataset
        """
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.memory_monitor = MemoryMonitor()
        
        print("Initializing SC_sylv Feature Extraction Tester (Pipeline Separation)")
    ...
    def test_data_loading(self) -> np.ndarray:
        ...
        print(f"  Test labels shape: {test_labels.shape} (expected: (n_subjects, 6))")
        assert test_labels.shape[1] == 6, f"Wrong labels shape: {test_labels.shape}"
    ...
    def test_25d_preprocessing(self, sample_volume: np.ndarray) -> dict:
        ...
        standard_slices = {}
        expected_standard = {'sagittal': 38, 'coronal': 36, 'axial': 49}
        ...
        slices_25d = {}
        expected_25d = {'sagittal': 18, 'coronal': 17, 'axial': 24}
    ...
    def test_spatial_aggregation(self, feature_maps_dict: dict) -> dict:
        ...
        # Test pooling only (no concat for SC_sylv)
    ...
    def test_standalone_pca_processing(self, aggregated_features: dict) -> None:
        """
        Test Standalone PCA processing with SC_sylv features including PCA_99.
    ...
    def test_memory_estimation_sc(self) -> None:
        """
        Test memory and storage requirements estimation for SC_sylv.
        """
        print("\n7. Testing Memory and Storage Estimation (SC_sylv)")
        
        n_subjects = 883  # SC_sylv labeled subjects
        n_splits = 6  # 5 train/val + 1 test
        
        print("  Estimating SC_sylv storage requirements...")
        ...
        n_training_subjects = 707  # SC_sylv training subjects from 5 folds
        ...
        if training_memory_pooling < available_ram * 0.8:
            ...
            print(f"  Memory may need optimization: {training_memory_pooling:.3f}GB vs {available_ram*0.8:.1f}GB available")
        ...
        if total_all_configs < storage_limit:
            ...
            print(f"  Total storage may exceed limit: {total_all_configs:.2f}GB > {storage_limit}GB")
    
    def test_sc_dimensions_validation(self) -> None:
        """
        Test and validate SC_sylv dimensions with overlapping 2.5D.
        """
        print("\n8. Testing SC_sylv Dimensions Validation")
        
        volume_shape = (38, 36, 49)
        print(f"  Volume shape: {volume_shape}")
        
        # Expected groups per axis with overlapping step=2
        expected_groups = {
            'sagittal': 18,   # range(0, 36, 2) → 18 groups
            'coronal': 17,    # range(0, 34, 2) → 17 groups
            'axial': 24       # range(0, 47, 2) → 24 groups
        }
        ...
        print(f"  Total groups: {total_groups} (should be 59)")
        ...
        print("  SC_sylv dimensions validation OK")
    
    def run_all_tests(self) -> None:
        """
        Run complete test suite for SC_sylv feature extraction with PCA separation.
        """
        print("=" * 80)
        print("SC_SYLV FEATURE EXTRACTION PIPELINE TESTS (SEPARATED PCA)")
        print("=" * 80)
        ...
        try:
            ...
            self.test_memory_estimation_sc()
            self.test_sc_dimensions_validation()
            ...
            print("SC_sylv pipeline ready for full execution!")
            print("Key features validated:")
            print("  - DINOv2 Giant: 16x16 patches (256 total) with 1536D features")
            print("  - SC_sylv dimensions: (38, 36, 49) with overlapping 2.5D step=2")
            print("  - Overlapping groups: 18+17+24 = 59 groups total")
            ...
            print("  - Memory optimization: < 4 GB peak usage")
            print("  - Regression task: 6D target labels")
            print("  - Left hemisphere: Sulcus Central analysis")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test SC_sylv feature extraction pipeline")
    parser.add_argument('--data-path', type=str, default='../../crops/2mm/S.C.-sylv./',
                       help='Path to SC_sylv dataset')
    ...
    if not Path(args.data_path).exists():
        ...
        print("Please provide correct path to SC_sylv dataset")
        sys.exit(1)
    
    # Run tests
    tester = SCFeatureExtractionTester(args.data_path)
