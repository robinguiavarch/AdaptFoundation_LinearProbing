"""
Pipeline 1: Feature extraction from DINOv2 Giant for SC_sylv dataset.
...
class SCFeatureExtractionPipeline:
    """
    Pipeline 1: Raw feature extraction for SC_sylv dataset using DINOv2 Giant.
    ...
    Attributes:
        ...
        data_loader (SCDataLoader): Data loader for SC_sylv dataset
    ...
    def __init__(self, config_path: str):
        """
        Initialize SC_sylv feature extraction pipeline.
        ...
        self.data_loader = SCDataLoader(self.config['dataset']['data_path'])
        ...
        print(f"SC_sylv feature extraction pipeline initialized:")
    ...
    def extract_single_subject(self, skeleton_volume: np.ndarray, variant_config: dict) -> np.ndarray:
        ...
        Args:
            skeleton_volume (np.ndarray): 3D skeleton volume with shape (38, 36, 49)
    ...
    def extract_variant_features(self, variant_config: dict) -> None:
        ...
        for split_name in all_splits:
            ...
            metadata_df = pd.DataFrame({'Subject': subjects, 'Labels': [label.tolist() for label in labels]})
    ...
    def _save_variant_metadata(self, variant_config: dict, output_dir: Path) -> None:
        ...
        metadata = {
            ...
            'extraction_info': {
                'pipeline': 'sc_feature_extraction',
    ...
    def run_extraction(self) -> None:
        """
        Run Pipeline 1: Extract raw feature map features for all SC_sylv variants.
        """
        configurations = self.config['configurations']
        
        print("=" * 80)
        print("PIPELINE 1: SC_SYLV RAW FEATURE EXTRACTION")
        print("=" * 80)
...
def main():
    """
    Main entry point for SC_sylv feature extraction pipeline.
    """
    parser = argparse.ArgumentParser(
        description="SC_sylv feature extraction - Pipeline 1",
    ...
    try:
        pipeline = SCFeatureExtractionPipeline(str(config_path))
