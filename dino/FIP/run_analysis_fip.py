"""
Analysis script for F.I.P. pooling classification results.

This script analyzes the 8 combinations from feature_extracted_fip_dinov2 directory
...
class FIPAnalyzer:
    """
    Analyzer for F.I.P. pooling classification results.
    
    Processes results from all 8 combinations (2 variants × 4 PCA modes)
    and generates comparative analysis tables and CSV files.
    
    Attributes:
        features_base_path (Path): Path to feature_extracted_fip_dinov2 directory
    """
    
    def __init__(self, features_base_path: str):
        """
        Initialize the F.I.P. analyzer.
        
        Args:
            features_base_path (str): Path to feature_extracted_fip_dinov2 directory
        """
    ...
    def create_complete_table(self, df: pd.DataFrame, metric_type: str = "test") -> plt.Figure:
        """
        Create styled table for complete F.I.P. configurations ranking.
        ...
        if df.empty:
            ...
            ax.text(0.5, 0.5, 'No F.I.P. Data Available', ha='center', va='center', fontsize=16)
            ax.set_title(f'F.I.P. Pooling - Logistic Regression ({metric_type.upper()})')
            ax.axis('off')
            return fig
        ...
        ax.set_title(f'F.I.P. Pooling - Logistic Regression ({n_configs} Configurations)\n'
    ...
    def run_analysis(self) -> Tuple[pd.DataFrame, List[plt.Figure]]:
        """
        Execute complete F.I.P. analysis workflow.
...
def main():
    """
    Main entry point for F.I.P. pooling analysis script.
    """
    parser = argparse.ArgumentParser(
        description="Run analysis of F.I.P. pooling classification results"
    )
    
    parser.add_argument(
        "--features-path",
        type=str,
        default="feature_extracted_fip_dinov2",
        help="Path to feature_extracted_fip_dinov2 directory"
    )
    
    parser.add_argument(
        "--output-dir",
        type=str,
        default="analysis_results_fip",
    ...
    print("F.I.P. Pooling Analysis")
    print(f"Features path: {features_path}")
    print(f"Output directory: {output_dir}")
    
    analyzer = FIPAnalyzer(str(features_path))
    
    print("\nExecuting F.I.P. analysis workflow...")
    df_all, figures = analyzer.run_analysis()
    
    if not df_all.empty:
        output_file_logistic = output_dir / "analysis_results_fip_logistic.csv"
        df_all.to_csv(output_file_logistic, index=False)
        print(f"Saved logistic results: {output_file_logistic}")
    
    figure_names = [
        "complete_fip_table_test",
        "complete_fip_table_cv"
    ]
    ...
    if not df_all.empty:
        print(f"\nF.I.P. Pooling Analysis Summary:")
    ...
    print(f"\nF.I.P. Pooling analysis completed successfully!")
    print(f"Tables saved as: complete_fip_table_test.png & complete_fip_table_cv.png")
