"""
Pipeline 1: Feature extraction from DINOv2 Giant for LARGE_CINGULATE dataset.
...
class LCFeatureExtractionPipeline:
    """
    Pipeline 1: Raw feature extraction for LARGE_CINGULATE dataset using DINOv2 Giant.
    ...
    def __init__(self, config_path: str):
        """
        Initialize LARGE_CINGULATE feature extraction pipeline.
        ...
        self.data_loader = LCDataLoader(self.config['dataset']['data_path'])
        ...
        print(f"LARGE_CINGULATE feature extraction pipeline initialized:")
    ...
    def extract_single_subject(self, skeleton_volume: np.ndarray, variant_config: dict) -> np.ndarray:
        ...
        Args:
            skeleton_volume (np.ndarray): 3D skeleton volume with shape (18, 73, 57)
    ...
    def _save_variant_metadata(self, variant_config: dict, output_dir: Path) -> None:
        ...
        metadata = {
            ...
            'extraction_info': {
                'pipeline': 'lc_feature_extraction',
    ...
    def run_extraction(self) -> None:
        """
        Run Pipeline 1: Extract raw feature map features for all LARGE_CINGULATE variants.
        """
        configurations = self.config['configurations']
        
        print("=" * 80)
        print("PIPELINE 1: LARGE_CINGULATE RAW FEATURE EXTRACTION")
        print("=" * 80)
...
def main():
    """
    Main entry point for LARGE_CINGULATE feature extraction pipeline.
    """
    parser = argparse.ArgumentParser(
        description="LARGE_CINGULATE feature extraction - Pipeline 1",
    ...
    try:
        pipeline = LCFeatureExtractionPipeline(str(config_path))
