"""
Analysis script for LARGE_CINGULATE pooling classification results.

This script analyzes the 8 combinations from feature_extracted_lc_dinov2 directory
...
class LCAnalyzer:
    """
    Analyzer for LARGE_CINGULATE pooling classification results.
    
    Processes results from all 8 combinations (2 variants × 4 PCA modes)
    and generates comparative analysis tables and CSV files.
    
    Attributes:
        features_base_path (Path): Path to feature_extracted_lc_dinov2 directory
    """
    
    def __init__(self, features_base_path: str):
        """
        Initialize the LARGE_CINGULATE analyzer.
        
        Args:
            features_base_path (str): Path to feature_extracted_lc_dinov2 directory
        """
    ...
    def create_complete_table(self, df: pd.DataFrame, metric_type: str = "test") -> plt.Figure:
        """
        Create styled table for complete LARGE_CINGULATE configurations ranking.
        ...
        if df.empty:
            ...
            ax.text(0.5, 0.5, 'No LARGE_CINGULATE Data Available', ha='center', va='center', fontsize=16)
            ax.set_title(f'LARGE_CINGULATE Pooling - Logistic Regression ({metric_type.upper()})')
            ax.axis('off')
            return fig
        ...
        ax.set_title(f'LARGE_CINGULATE Pooling - Logistic Regression ({n_configs} Configurations)\n'
    ...
    def run_analysis(self) -> Tuple[pd.DataFrame, List[plt.Figure]]:
        """
        Execute complete LARGE_CINGULATE analysis workflow.
...
def main():
    """
    Main entry point for LARGE_CINGULATE pooling analysis script.
    """
    parser = argparse.ArgumentParser(
        description="Run analysis of LARGE_CINGULATE pooling classification results"
    )
    
    parser.add_argument(
        "--features-path",
        type=str,
        default="feature_extracted_lc_dinov2",
        help="Path to feature_extracted_lc_dinov2 directory"
    )
    
    parser.add_argument(
        "--output-dir",
        type=str,
        default="analysis_results_lc",
    ...
    print("LARGE_CINGULATE Pooling Analysis")
    print(f"Features path: {features_path}")
    print(f"Output directory: {output_dir}")
    
    analyzer = LCAnalyzer(str(features_path))
    
    print("\nExecuting LARGE_CINGULATE analysis workflow...")
    df_all, figures = analyzer.run_analysis()
    
    if not df_all.empty:
        output_file_logistic = output_dir / "analysis_results_lc_logistic.csv"
        df_all.to_csv(output_file_logistic, index=False)
        print(f"Saved logistic results: {output_file_logistic}")
    
    figure_names = [
        "complete_lc_table_test",
        "complete_lc_table_cv"
    ]
    ...
    if not df_all.empty:
        print(f"\nLARGE_CINGULATE Pooling Analysis Summary:")
    ...
    print(f"\nLARGE_CINGULATE Pooling analysis completed successfully!")
    print(f"Tables saved as: complete_lc_table_test.png & complete_lc_table_cv.png")
