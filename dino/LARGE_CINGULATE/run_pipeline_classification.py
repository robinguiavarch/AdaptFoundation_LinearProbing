"""
Classification pipeline for LARGE_CINGULATE binary classification with CV investigation.

This script runs linear probing classification on DINOv2 Giant pooling features
...
class LCLinearProber:
    """
    Linear probing classifier for LARGE_CINGULATE binary classification with CV investigation.
    
    Evaluates logistic regression performance on DINOv2 Giant pooling features
    for Right_PCS classification with detailed fold-level analysis.
    ...
    def __init__(self, features_base_path: str, investigation_config: Dict = None, 
        ...
        """
        Initialize the LARGE_CINGULATE linear probing classifier.
    ...
    def _load_cv_data_with_groups(self, variant_path: Path) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        Critical: Preserves pre-stratified LARGE_CINGULATE folds by assigning group = fold_id.
    ...
    def _save_consolidated_results(self, all_results: Dict, total_time: float) -> None:
        ...
        """
        consolidated_file = self.features_base_path / "lc_classification_results.json"
        
        investigation_summary = self._generate_investigation_summary(all_results)
        
        summary = {
            'experiment_info': {
                'pipeline': 'LARGE_CINGULATE Binary Classification with CV Investigation',
                'model': 'DINOv2 Giant (vitg14)',
                'classifier': 'Logistic Regression',
                'task': 'Right_PCS binary classification',
...
def main():
    """
    Main entry point for LARGE_CINGULATE binary classification pipeline with investigation.
    """
    parser = argparse.ArgumentParser(
        description="LARGE_CINGULATE Binary Classification Pipeline with CV Investigation"
    )
    
    parser.add_argument(
        "--config",
        type=str,
        default="classification_lc.yaml",
    ...
    print("LARGE_CINGULATE Binary Classification Pipeline with CV Investigation")
    print(f"Features path: {features_path}")
    print(f"Configuration: {config_path}")
    print(f"Investigation enabled: {config.get('investigation', {}).get('enabled', False)}")
    
    try:
        prober = LCLinearProber(
            str(features_path),
