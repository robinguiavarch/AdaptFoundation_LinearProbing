"""
Test script for LARGE_CINGULATE feature maps extraction pipeline with separated PCA.

This script validates all components of the LARGE_CINGULATE feature extraction approach
...
from dino.LARGE_CINGULATE.feature_extraction_core import (
...
class LCFeatureExtractionTester:
    """
    Tests all components of the LARGE_CINGULATE feature extraction pipeline.
    ...
    def __init__(self, data_path: str = "../../crops/2mm/LARGE_CINGULATE./"):
        ...
        print("Initializing LARGE_CINGULATE Feature Extraction Tester (Pipeline Separation)")
    ...
    def test_25d_preprocessing(self, sample_volume: np.ndarray) -> dict:
        ...
        standard_slices = {}
        expected_standard = {'sagittal': 18, 'coronal': 73, 'axial': 57}
        ...
        slices_25d = {}
        expected_25d = {'sagittal': 8, 'coronal': 36, 'axial': 28}
    ...
    def test_spatial_aggregation(self, feature_maps_dict: dict) -> dict:
        ...
        # Test pooling only (no concat for LARGE_CINGULATE)
    ...
    def test_standalone_pca_processing(self, aggregated_features: dict) -> None:
        """
        Test Standalone PCA processing with LARGE_CINGULATE features including PCA_99.
    ...
    def test_memory_estimation_lc(self) -> None:
        """
        Test memory and storage requirements estimation for LARGE_CINGULATE.
        """
        print("\n7. Testing Memory and Storage Estimation (LARGE_CINGULATE)")
        
        n_subjects = 341  # LARGE_CINGULATE labeled subjects
        n_splits = 6  # 5 train/val + 1 test
        
        print("  Estimating LARGE_CINGULATE storage requirements...")
        ...
        n_training_subjects = 273  # LARGE_CINGULATE training subjects from 5 folds
    ...
    def test_lc_dimensions_validation(self) -> None:
        """
        Test and validate LARGE_CINGULATE dimensions with overlapping 2.5D.
        """
        print("\n8. Testing LARGE_CINGULATE Dimensions Validation")
        
        volume_shape = (18, 73, 57)
        print(f"  Volume shape: {volume_shape}")
        
        # Expected groups per axis with overlapping step=2
        expected_groups = {
            'sagittal': 8,    # range(0, 16, 2) → 8 groups
            'coronal': 36,    # range(0, 71, 2) → 36 groups
            'axial': 28       # range(0, 55, 2) → 28 groups
        }
        ...
        print(f"  Total groups: {total_groups} (should be 72)")
        ...
        print("  ✅ LARGE_CINGULATE dimensions validation OK")
    
    def run_all_tests(self) -> None:
        """
        Run complete test suite for LARGE_CINGULATE feature extraction with PCA separation.
        """
        print("=" * 80)
        print("LARGE_CINGULATE FEATURE EXTRACTION PIPELINE TESTS (SEPARATED PCA)")
        print("=" * 80)
        ...
        try:
            ...
            self.test_memory_estimation_lc()
            self.test_lc_dimensions_validation()
            ...
            print("  - LARGE_CINGULATE dimensions: (18, 73, 57) with overlapping 2.5D step=2")
            print("  - Overlapping groups: 8+36+28 = 72 groups total")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test LARGE_CINGULATE feature extraction pipeline")
    ...
    # Run tests
    tester = LCFeatureExtractionTester(args.data_path)
