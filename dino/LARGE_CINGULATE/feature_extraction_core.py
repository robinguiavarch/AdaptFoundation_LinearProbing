...
class Method25D:
    ...
    def create_25d_slices_adaptive(self, volume_3d: np.ndarray, axis: str) -> torch.Tensor:
        """
        Create 2.5D slices with adaptive overlapping grouping for LARGE_CINGULATE dimensions.
        
        Args:
            volume_3d (np.ndarray): Input volume with shape (18, 73, 57)
            axis (str): Slicing axis ('axial', 'coronal', 'sagittal')
        
        Returns:
            torch.Tensor: Processed slices with overlapping step=2
        """
        if axis == 'sagittal':  # X axis - 18 slices (0-17)
            # Create overlapping groups: [0,1,2], [2,3,4], [4,5,6], ..., [14,15,16]
            # Max start_idx = 14 (pour avoir [14,15,16])
            slices_2d = []
            for start_idx in range(0, 16, 2):  # 0, 2, 4, ..., 14 → 8 groups
        ...
        elif axis == 'coronal':  # Y axis - 73 slices (0-72)
            # Create overlapping groups: [0,1,2], [2,3,4], [4,5,6], ..., [70,71,72]
            # Max start_idx = 70 (pour avoir [70,71,72])
            slices_2d = []
            for start_idx in range(0, 71, 2):  # 0, 2, 4, ..., 70 → 36 groups
                group_slices = np.stack([
                    volume_3d[:, start_idx, :],
                    volume_3d[:, start_idx + 1, :], 
                    volume_3d[:, start_idx + 2, :]
        ...
        elif axis == 'axial':  # Z axis - 57 slices (0-56)
            # Create overlapping groups: [0,1,2], [2,3,4], [4,5,6], ..., [54,55,56]
            # Max start_idx = 54 (pour avoir [54,55,56])
            slices_2d = []
            for start_idx in range(0, 55, 2):  # 0, 2, 4, ..., 54 → 28 groups
                group_slices = np.stack([
                    volume_3d[:, :, start_idx],
                    volume_3d[:, :, start_idx + 1], 
                    volume_3d[:, :, start_idx + 2]
    ...
    def create_standard_slices(self, volume_3d: np.ndarray, axis: str) -> torch.Tensor:
        ...
        """
        if axis == 'sagittal':
            slices_2d = [volume_3d[i, :, :] for i in range(18)]
        elif axis == 'coronal':
            slices_2d = [volume_3d[:, i, :] for i in range(73)]
        elif axis == 'axial':
            slices_2d = [volume_3d[:, :, i] for i in range(57)]
        else:
            raise ValueError(f"Unknown axis: {axis}")
