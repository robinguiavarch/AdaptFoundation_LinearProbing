#!/usr/bin/env python3
"""
PCA dimensionality reduction pipeline for Point-M2AE features.

This script applies PCA reduction to Point-M2AE features with two approaches:
feat_mean (384D) and feat_mean_max (768D) to target dimensions 32D and 256D.
"""
...
class PointM2AEDimensionalityReducer:
    """
    Handles dimensionality reduction of Point-M2AE features using PCA.
    
    Supports feat_mean (384D) and feat_mean_max (768D) approaches with
    fixed component reduction to 32D and 256D for downstream classification.
    """
    
    def __init__(self, features_base_path: str, n_components: int):
        """
        Initialize the Point-M2AE dimensionality reducer.
        
        Args:
            features_base_path (str): Base path to feature_extraction_point_m2ae directory
        ...
        print(f"PointM2AEDimensionalityReducer initialized:")
    ...
    def _load_split_features(self, approach_name: str, split_name: str) -> Tuple[np.ndarray, pd.DataFrame]:
        ...
        Args:
            approach_name (str): Approach name ('feat_mean' or 'feat_mean_max')
    ...
    def process_approach(self, approach_name: str) -> Dict:
        """
        Complete PCA processing for an approach: fit on training, transform all splits.
        
        Args:
            approach_name (str): Approach name to process
...
def validate_pca_task(task: Dict[str, Any], features_base_path: str) -> bool:
    ...
    Args:
        ...
        features_base_path (str): Path to feature_extraction_point_m2ae directory
...
def execute_pca_task(task: Dict[str, Any], features_base_path: str) -> Dict[str, Any]:
    ...
    try:
        reducer = PointM2AEDimensionalityReducer(
...
def run_yaml_pca_reduction(config_file: str) -> None:
    ...
    """
    print("=" * 60)
    print("POINT-M2AE PCA DIMENSIONALITY REDUCTION")
    print("=" * 60)
    ...
    if successful > 0:
        print(f"\nStructure created:")
        print(f"feature_extraction_point_m2ae/")
...
def run_single_approach(features_base_path: str, approach_name: str, n_components: int) -> None:
    """
    Run PCA reduction for a single approach.
    ...
    """
    print("=" * 60)
    print("POINT-M2AE PCA - SINGLE APPROACH")
    print("=" * 60)
    print(f"Features base path: {features_base_path}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Apply PCA dimensionality reduction to Point-M2AE features"
    )
    
    parser.add_argument(
        "--config",
        type=str,
        default="pca_reduction_m2ae.yaml",
    ...
    parser.add_argument(
        ...
        help="Process only a specific approach"
    )
    ...
    parser.add_argument(
        ...
        help="Path to feature_extraction_point_m2ae directory"
    )
    ...
    if args.approach:
        print("=== Single Approach PCA Processing ===")
