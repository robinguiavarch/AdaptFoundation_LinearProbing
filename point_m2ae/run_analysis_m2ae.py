"""
Analysis script for Point-M2AE feature approach classification results.

This script analyzes the 6 combinations from point_m2ae directory
and generates comparative tables for feature approaches evaluation.
"""
...
class PointM2AEAnalyzer:
    """
    Analyzer for Point-M2AE feature approach classification results.
    
    Processes results from all 6 combinations (2 approaches × 3 modes)
    and generates comparative analysis tables for feature approaches.
    ...
    def __init__(self, results_base_path: str):
        """
        Initialize the Point-M2AE analyzer.
            ...
            for mode in modes:
                ...
                if result_file.exists():
                    try:
                        ...
                        approach_type = 'mean' if approach == 'feat_mean' else 'mean_max'
                        
                        if mode == 'raw_features':
                            feature_dim = 384 if approach == 'feat_mean' else 768
                            mode_clean = mode
                        else:
    ...
    def filter_by_approach_type(self, df: pd.DataFrame, approach_type: str = 'all') -> pd.DataFrame:
        ...
        Args:
            ...
            approach_type (str): Approach type ('mean', 'mean_max', or 'all')
    ...
    def create_approaches_table(self, df_filtered: pd.DataFrame, approach_type: str, 
        ...
        """
        Create styled table for Point-M2AE feature approaches ranking.
        
        Args:
            df_filtered (pd.DataFrame): Filtered dataset by approach type
            approach_type (str): Approach type ('all', 'mean', or 'mean_max')
        ...
        if df_filtered.empty:
            ...
            ax.set_title(f'Point-M2AE Feature Approaches - Logistic Regression ({metric_type.upper()})')
            ax.axis('off')
            return fig
        ...
        # Color scheme for Point-M2AE (Blue/Cyan for hierarchical features)
        header_color = '#2196F3'  # Blue for Point-M2AE
        ...
        for i in range(1, len(table_data) + 1):
            ...
            for j in range(len(columns)):
                ...
                if j == 0:  # Rank column
                    ...
                    if 'Mean + Max' in table_data[i-1][2]:
        ...
        ax.set_title(f'Point-M2AE Feature Approaches - Logistic Regression ({n_configs} Configurations)\n'
                    f'Modes: {modes_str} - Ranked by {metric_display} ({approach_type_display})', 
                    fontsize=16, fontweight='bold', pad=30)
        
        fig.text(0.5, 0.02, f'Point-M2AE hierarchical encoder features - Spatial aggregation approaches - Ranked by {metric_display}', 
    ...
    def run_analysis(self) -> Tuple[pd.DataFrame, List[plt.Figure]]:
        """
        Execute complete Point-M2AE feature approaches analysis workflow.
...
def main():
    """
    Main entry point for Point-M2AE feature approaches analysis script.
    """
    parser = argparse.ArgumentParser(
        description="Run analysis of Point-M2AE feature approach classification results"
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="analysis_results_point_m2ae",
    ...
    print("Point-M2AE Feature Approaches Analysis")
    print(f"Results path: {results_path}")
    print(f"Output directory: {output_dir}")
    
    analyzer = PointM2AEAnalyzer(str(results_path))
    
    print("\nExecuting Point-M2AE feature approaches analysis workflow...")
    df_all, figures = analyzer.run_analysis()
    
    if not df_all.empty:
        output_file_logistic = output_dir / "analysis_results_point_m2ae_logistic.csv"
        df_all.to_csv(output_file_logistic, index=False)
        print(f"Saved results: {output_file_logistic}")
    
    figure_names = [
        "point_m2ae_approaches_table_test",
        "point_m2ae_approaches_table_cv"
    ]
    ...
    if not df_all.empty:
        print(f"\nPoint-M2AE Feature Approaches Analysis Summary:")
        ...
        # Approach comparison
        mean_results = df_all[df_all['approach'] == 'feat_mean']
        mean_max_results = df_all[df_all['approach'] == 'feat_mean_max']
        
        if not mean_results.empty and not mean_max_results.empty:
            print(f"\nMean vs Mean+Max Approaches Comparison:")
            print(f"Mean approach Test ROC-AUC: {mean_results['test_roc_auc'].mean():.4f}")
            print(f"Mean+Max approach Test ROC-AUC: {mean_max_results['test_roc_auc'].mean():.4f}")
            print(f"Mean approach CV ROC-AUC: {mean_results['cv_roc_auc'].mean():.4f}")
            print(f"Mean+Max approach CV ROC-AUC: {mean_max_results['cv_roc_auc'].mean():.4f}")
    
    print(f"\nPoint-M2AE feature approaches analysis completed successfully!")
