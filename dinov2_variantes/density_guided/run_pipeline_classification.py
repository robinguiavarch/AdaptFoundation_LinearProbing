"""
Classification pipeline for DINOv2 Density-Guided CLS Token variants.

This script runs linear probing classification on density-guided CLS token extracted features
...
class DensityGuidedLinearProber:
    """
    Linear probing classifier for DINOv2 Density-Guided CLS Token variants evaluation.
    
    Evaluates logistic regression performance on all variant/PCA combinations
    from the density_guided_features directory structure.
    ...
    def __init__(self, features_base_path: str, random_state: int = 42, n_jobs: int = -1):
        """
        Initialize the DINOv2 Density-Guided linear probing classifier.
    ...
    def train_variant_classifier(self, variant_name: str, pca_mode: str, 
        ...
        Args:
            variant_name (str): Variant name (e.g., 'central_uniform_concat')
    ...
    def _save_consolidated_results(self, all_results: Dict, total_time: float) -> None:
        ...
        """
        consolidated_file = self.features_base_path / "density_guided_classification_results.json"
        
        summary = {
            'experiment_info': {
                'pipeline': 'DINOv2 Density-Guided Classification',
...
def main():
    """
    Main entry point for DINOv2 Density-Guided Classification Pipeline.
    """
    parser = argparse.ArgumentParser(
        description="DINOv2 Density-Guided Classification Pipeline"
    )
    
    parser.add_argument(
        "--config",
        type=str,
        default="dinov2_variantes/density_guided/classification.yaml",
    ...
    print("DINOv2 Density-Guided Classification Pipeline")
    print(f"Model: DINOv2 Giant (vitg14)")
    print(f"Feature type: Density-optimized CLS tokens (1536D)")
    print(f"Features path: {features_path}")
    print(f"Configuration: {config_path}")
    
    try:
        prober = DensityGuidedLinearProber(
            str(features_path),
