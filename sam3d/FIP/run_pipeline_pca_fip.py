#!/usr/bin/env python3
"""
PCA dimensionality reduction pipeline for F.I.P. dataset features.

This script applies PCA reduction to SAM-Med3D features extracted from F.I.P. dataset
with support for fixed components and variance threshold modes.

Usage:
python sam3d/FIP/run_pipeline_pca_fip.py
python sam3d/FIP/run_pipeline_pca_fip.py --config pca_reduction_fip.yaml
"""
...
try:
    print("Initializing PCA pipeline for F.I.P. features")
...
class FIPDimensionalityReducer:
    """
    Handles dimensionality reduction of F.I.P. features using PCA.
    
    Supports both fixed components and variance threshold modes for F.I.P. dataset.
    
    Attributes:
        features_base_path (Path): Base path to feature_extraction_sam3d_fip directory
    ...
    def __init__(self, features_base_path: str, 
        ...
        """
        Initialize F.I.P. dimensionality reducer.
        
        Args:
            features_base_path (str): Base path to feature_extraction_sam3d_fip directory
        ...
        print(f"FIP Dimensionality Reducer initialized")
    ...
    def fit_pca(self) -> Dict:
        ...
        """
        print("Fitting PCA on F.I.P. training data")
    ...
    def process_pca_reduction(self) -> Dict:
        ...
        """
        print(f"\n{'='*60}")
        print("Processing F.I.P. PCA reduction")
        print(f"{'='*60}")
        ...
        print(f"PCA processing completed for F.I.P. dataset")
...
def get_pca_execution_plan(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Generate PCA execution plan from YAML configuration for F.I.P. dataset.
    ...
    print(f"Generated {len(tasks)} F.I.P. PCA tasks")
...
def execute_pca_task(task: Dict[str, Any], features_base_path: str, 
    ...
    if processing_config.get('detailed_logging', True):
        print(f"Executing F.I.P. PCA: {configuration} | {task['description']}")
    
    try:
        if task['mode'] == 'fixed':
            reducer = FIPDimensionalityReducer(
...
def run_yaml_pca_reduction(config_file: str) -> None:
    ...
    """
    print("=" * 80)
    print("F.I.P. PCA DIMENSIONALITY REDUCTION")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("F.I.P. PCA PROCESSING COMPLETED")
    print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Apply PCA dimensionality reduction to F.I.P. SAM-Med3D features",
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="pca_reduction_fip.yaml",
