#!/usr/bin/env python3
"""
Test script for F.I.P. SAM-Med3D feature extraction and PCA pipeline.
...
class FIPFeatureExtractionPCATester:
    """
    Tests all components of the F.I.P. SAM-Med3D feature extraction and PCA pipeline.
    ...
    def __init__(self, data_path: str = "crops/2mm/F.I.P."):
        ...
        print("Initializing F.I.P. SAM-Med3D Feature Extraction and PCA Tester")
        print(f"Device: {self.device}")
        
        try:
            self.data_loader = FIPDataLoader(data_path)
            print(f"F.I.P. dataset loaded: {len(self.data_loader.skeletons)} volumes")
        except Exception as e:
            print(f"ERROR loading F.I.P. dataset: {e}")
            sys.exit(1)
    
    def test_fip_data_loading(self) -> np.ndarray:
        """
        Test F.I.P. data loading and return sample volume.
        
        Returns:
            np.ndarray: Sample 3D volume for testing
        """
        print("\n1. Testing F.I.P. Data Loading")
        
        sample_volume = self.data_loader.skeletons[0]
        expected_shape = (39, 45, 44)  # F.I.P. specific dimensions
        expected_subjects = 390  # F.I.P. labeled subjects
    ...
    def test_sam3d_extractor_loading(self) -> SAMMed3DStandardExtractor:
        ...
        try:
            extractor = SAMMed3DStandardExtractor(
                config_path="sam3d/FIP/feature_extraction_fip.yaml"
            )
            ...
            assert model_info['dataset'] == 'F.I.P.', "Wrong dataset"
    ...
    def test_feature_extraction(self, extractor: SAMMed3DStandardExtractor, 
        ...
        """
        Test feature extraction with F.I.P. volume.
        
        Args:
            extractor (SAMMed3DStandardExtractor): Feature extractor
            sample_volume (np.ndarray): Sample F.I.P. volume
    ...
    def test_batch_processing(self, extractor: SAMMed3DStandardExtractor) -> np.ndarray:
        ...
        # Create test batch with F.I.P. volumes
        test_volumes = []
        for i in range(50):  # Augmenté de 5 à 50 échantillons
    ...
    def test_pca_pipeline(self, features: np.ndarray) -> None:
        ...
        pca_modes = [
            ...
            # Ces modes seront testés en production avec 390 sujets F.I.P.
        ]
    ...
    def test_memory_requirements(self) -> None:
        """
        Test memory and storage requirements for F.I.P. dataset.
        """
        ...
        print("  Estimating storage requirements for F.I.P.")
        ...
        # Storage limit check
        storage_limit = 10  # GB (reasonable for F.I.P.)
    ...
    def test_pipeline_integration(self) -> None:
        ...
        print("  Simulating complete F.I.P. pipeline:")
        print("    1. Feature extraction: F.I.P. volumes → SAM-Med3D → 196608D features")
        print("    2. PCA reduction: 196608D → 32D/256D/95%/99.5%")
        print("    3. Output structure: feature_extraction_sam3d_fip/flatten/PCA_{mode}/")
        
        # Test expected file structure
        expected_structure = {
            'feature_extraction_sam3d_fip/flatten/': [
    ...
    def run_all_tests(self) -> None:
        """
        Run complete test suite for F.I.P. feature extraction and PCA pipeline.
        """
        print("=" * 80)
        print("F.I.P. SAM-MED3D FEATURE EXTRACTION AND PCA PIPELINE TESTS")
        print("=" * 80)
        ...
        try:
            ...
            sample_volume = self.test_fip_data_loading()
            ...
            print(f"\n{'='*80}")
            print("ALL F.I.P. TESTS PASSED ✅")
            print(f"Total test time: {total_time:.1f}s")
            self.memory_monitor.print_memory_status("final")
            print("F.I.P. pipeline ready for full execution!")
            print()
            print("Key features validated:")
            print("  - F.I.P. dataset: 390 subjects, binary classification")
            print("  - Volume dimensions: (39,45,44) → (128,128,128)")
            ...
            print("  - Output structure: feature_extraction_sam3d_fip/flatten/")
            print("  - Memory estimation: ~1.6GB total storage")
            print(f"{'='*80}")
...
def main():
    """
    Main entry point for F.I.P. testing.
    """
    import argparse
    
    parser = argparse.ArgumentParser(description="Test F.I.P. SAM-Med3D feature extraction and PCA pipeline")
    ...
    if not Path(args.data_path).exists():
        print(f"F.I.P. data path not found: {args.data_path}")
        print("Please provide correct path to F.I.P. dataset")
        sys.exit(1)
    
    # Check if config exists
    config_path = Path("sam3d/FIP/feature_extraction_fip.yaml")
    if not config_path.exists():
        print(f"Configuration file not found: {config_path}")
        print("Please ensure F.I.P. configuration files are in place")
        sys.exit(1)
    
    # Run tests
    tester = FIPFeatureExtractionPCATester(args.data_path)
