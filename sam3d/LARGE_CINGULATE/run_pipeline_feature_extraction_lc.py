#!/usr/bin/env python3
"""
Feature extraction pipeline for LARGE_CINGULATE dataset using SAM-Med3D standard model.

This script extracts features from LARGE_CINGULATE 3D skeletal volumes using SAM-Med3D turbo
with standard flatten aggregation for binary classification task.

Usage:
python sam3d/LARGE_CINGULATE/run_pipeline_feature_extraction_lc.py
python sam3d/LARGE_CINGULATE/run_pipeline_feature_extraction_lc.py --config feature_extraction_lc.yaml
"""
...
class LCFeatureDatasetSaver:
    """
    Feature dataset saver for LARGE_CINGULATE dataset with simplified structure.
    
    Creates structure: feature_extraction_sam3d_lc/flatten/
    ...
    Attributes:
        ...
        data_loader (LCDataLoader): Data loader for LARGE_CINGULATE dataset
    """
    
    def __init__(self, data_path: str, output_base_path: str):
        """
        Initialize LARGE_CINGULATE feature dataset saver.
        ...
        print(f"LARGE_CINGULATE Feature Dataset Saver initialized")
    ...
    def save_features(self, extractor: SAMMed3DStandardExtractor) -> None:
        """
        Extract and save features for LARGE_CINGULATE dataset.
        
        Args:
            extractor (SAMMed3DStandardExtractor): Feature extractor instance
        """
        print("Processing LARGE_CINGULATE dataset with flatten aggregation")
        ...
        if total_subjects > 0:
            ...
            print(f"LARGE_CINGULATE feature extraction completed")
...
def print_pipeline_summary(config: Dict) -> None:
    ...
    print("=" * 80)
    print("SAM-MED3D FEATURE EXTRACTION FOR LARGE_CINGULATE DATASET")
    print("=" * 80)
...
def run_feature_extraction(config_path: str) -> None:
    """
    Run complete feature extraction pipeline for LARGE_CINGULATE dataset.
    
    Args:
        config_path (str): Path to configuration YAML file
    """
    print("Loading LARGE_CINGULATE feature extraction configuration")
    ...
    print("Initializing LARGE_CINGULATE feature dataset saver")
    saver = LCFeatureDatasetSaver(
        data_path=data_path,
    ...
    try:
        ...
        print(f"\n{'='*80}")
        print("LARGE_CINGULATE FEATURE EXTRACTION COMPLETED")
        print(f"{'='*80}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Feature extraction for LARGE_CINGULATE dataset using SAM-Med3D",
    ...
    parser.add_argument(
        '--config',
        type=str,
        default='feature_extraction_lc.yaml',
