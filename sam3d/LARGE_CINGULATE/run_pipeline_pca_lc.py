#!/usr/bin/env python3
"""
PCA dimensionality reduction pipeline for LARGE_CINGULATE dataset features.

This script applies PCA reduction to SAM-Med3D features extracted from LARGE_CINGULATE dataset
with support for fixed components and variance threshold modes.

Usage:
python sam3d/LARGE_CINGULATE/run_pipeline_pca_lc.py
python sam3d/LARGE_CINGULATE/run_pipeline_pca_lc.py --config pca_reduction_lc.yaml
"""
...
try:
    print("Initializing PCA pipeline for LARGE_CINGULATE features")
...
class LCDimensionalityReducer:
    """
    Handles dimensionality reduction of LARGE_CINGULATE features using PCA.
    
    Supports both fixed components and variance threshold modes for LARGE_CINGULATE dataset.
    
    Attributes:
        features_base_path (Path): Base path to feature_extraction_sam3d_lc directory
    ...
    def __init__(self, features_base_path: str, 
        ...
        """
        Initialize LARGE_CINGULATE dimensionality reducer.
        
        Args:
            features_base_path (str): Base path to feature_extraction_sam3d_lc directory
        ...
        print(f"LARGE_CINGULATE Dimensionality Reducer initialized")
    ...
    def fit_pca(self) -> Dict:
        ...
        """
        print("Fitting PCA on LARGE_CINGULATE training data")
    ...
    def process_pca_reduction(self) -> Dict:
        ...
        """
        print(f"\n{'='*60}")
        print("Processing LARGE_CINGULATE PCA reduction")
        print(f"{'='*60}")
        ...
        print(f"PCA processing completed for LARGE_CINGULATE dataset")
...
def get_pca_execution_plan(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Generate PCA execution plan from YAML configuration for LARGE_CINGULATE dataset.
    ...
    print(f"Generated {len(tasks)} LARGE_CINGULATE PCA tasks")
...
def execute_pca_task(task: Dict[str, Any], features_base_path: str, 
    ...
    if processing_config.get('detailed_logging', True):
        print(f"Executing LARGE_CINGULATE PCA: {configuration} | {task['description']}")
    
    try:
        if task['mode'] == 'fixed':
            reducer = LCDimensionalityReducer(
...
def run_yaml_pca_reduction(config_file: str) -> None:
    ...
    """
    print("=" * 80)
    print("LARGE_CINGULATE PCA DIMENSIONALITY REDUCTION")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("LARGE_CINGULATE PCA PROCESSING COMPLETED")
    print("=" * 80)
    ...
    if successful > 0:
        print(f"\nStructure created:")
        print(f"feature_extraction_sam3d_lc/flatten/")
        for mode in ['PCA_32', 'PCA_256', 'PCA_95', 'PCA_995']:
            print(f"  └── {mode}/")
        print(f"\nNext steps:")
        print(f"1. Run linear probing classification on PCA-reduced features")
        print(f"2. Evaluate LARGE_CINGULATE binary classification performance")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Apply PCA dimensionality reduction to LARGE_CINGULATE SAM-Med3D features",
