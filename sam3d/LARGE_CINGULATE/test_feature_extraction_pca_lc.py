#!/usr/bin/env python3
"""
Test script for LARGE_CINGULATE SAM-Med3D feature extraction and PCA pipeline.
...
from data.loader_lc import LCDataLoader
from sam3d.LARGE_CINGULATE.feature_extraction_core_lc import SAMMed3DStandardExtractor
...
class LCFeatureExtractionPCATester:
    """
    Tests all components of the LARGE_CINGULATE SAM-Med3D feature extraction and PCA pipeline.
    
    Attributes:
        data_loader (LCDataLoader): Data loader for testing
        device (torch.device): Computation device
        memory_monitor (MemoryMonitor): Memory usage monitor
    """
    
    def __init__(self, data_path: str = "crops/2mm/LARGE_CINGULATE."):
        """
        Initialize LARGE_CINGULATE tester.
        
        Args:
            data_path (str): Path to LARGE_CINGULATE dataset
        """
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.memory_monitor = MemoryMonitor()
        
        print("Initializing LARGE_CINGULATE SAM-Med3D Feature Extraction and PCA Tester")
        print(f"Device: {self.device}")
        
        try:
            self.data_loader = LCDataLoader(data_path)
            print(f"LARGE_CINGULATE dataset loaded: {len(self.data_loader.skeletons)} volumes")
        except Exception as e:
            print(f"ERROR loading LARGE_CINGULATE dataset: {e}")
            sys.exit(1)
    
    def test_lc_data_loading(self) -> np.ndarray:
        """
        Test LARGE_CINGULATE data loading and return sample volume.
        
        Returns:
            np.ndarray: Sample 3D volume for testing
        """
        print("\n1. Testing LARGE_CINGULATE Data Loading")
        
        sample_volume = self.data_loader.skeletons[0]
        expected_shape = (18, 73, 57)  # LARGE_CINGULATE specific dimensions
        expected_subjects = 341  # LARGE_CINGULATE labeled subjects
        ...
        print("  ✅ LARGE_CINGULATE data loading OK")
    ...
    def test_sam3d_extractor_loading(self) -> SAMMed3DStandardExtractor:
        ...
        try:
            extractor = SAMMed3DStandardExtractor(
                config_path="sam3d/LARGE_CINGULATE/feature_extraction_lc.yaml"
            )
            ...
            assert model_info['dataset'] == 'LARGE_CINGULATE', "Wrong dataset"
            assert model_info['task_type'] == 'binary_classification', "Wrong task type"
    ...
    def test_feature_extraction(self, extractor: SAMMed3DStandardExtractor, 
        ...
        """
        Test feature extraction with LARGE_CINGULATE volume.
        
        Args:
            extractor (SAMMed3DStandardExtractor): Feature extractor
            sample_volume (np.ndarray): Sample LARGE_CINGULATE volume
        ...
        try:
            ...
            features_np = features.cpu().numpy()  # Conversion explicite GPU → CPU → numpy
    ...
    def test_batch_processing(self, extractor: SAMMed3DStandardExtractor) -> np.ndarray:
        ...
        # Create test batch with LARGE_CINGULATE volumes
        test_volumes = []
    ...
    def test_pca_pipeline(self, features: np.ndarray) -> None:
        ...
        pca_modes = [
            ...
            # Ces modes seront testés en production avec 341 sujets LARGE_CINGULATE
        ]
    ...
    def test_memory_requirements(self) -> None:
        """
        Test memory and storage requirements for LARGE_CINGULATE dataset.
        """
        print("\n6. Testing Memory Requirements")
        
        n_subjects = 341  # LARGE_CINGULATE labeled subjects
        n_splits = 6  # 5 train/val + 1 test
        
        print("  Estimating storage requirements for LARGE_CINGULATE")
        ...
        # Storage limit check
        storage_limit = 10  # GB (reasonable for LARGE_CINGULATE)
    ...
    def test_pipeline_integration(self) -> None:
        ...
        print("  Simulating complete LARGE_CINGULATE pipeline:")
        print("    1. Feature extraction: LARGE_CINGULATE volumes → SAM-Med3D → 196608D features")
        print("    2. PCA reduction: 196608D → 32D/256D/95%/99.5%")
        print("    3. Output structure: feature_extraction_sam3d_lc/flatten/PCA_{mode}/")
        
        # Test expected file structure
        expected_structure = {
            'feature_extraction_sam3d_lc/flatten/': [
    ...
    def run_all_tests(self) -> None:
        """
        Run complete test suite for LARGE_CINGULATE feature extraction and PCA pipeline.
        """
        print("=" * 80)
        print("LARGE_CINGULATE SAM-MED3D FEATURE EXTRACTION AND PCA PIPELINE TESTS")
        print("=" * 80)
        ...
        try:
            ...
            sample_volume = self.test_lc_data_loading()
            ...
            print(f"\n{'='*80}")
            print("ALL LARGE_CINGULATE TESTS PASSED ✅")
            print(f"Total test time: {total_time:.1f}s")
            self.memory_monitor.print_memory_status("final")
            print("LARGE_CINGULATE pipeline ready for full execution!")
            print()
            print("Key features validated:")
            print("  - LARGE_CINGULATE dataset: 341 subjects, binary classification")
            print("  - Volume dimensions: (18,73,57) → (128,128,128)")
            ...
            print("  - Output structure: feature_extraction_sam3d_lc/flatten/")
            print("  - Memory estimation: ~1.4GB total storage")
            print(f"{'='*80}")
            
        except Exception as e:
            print(f"\n❌ LARGE_CINGULATE TEST FAILED: {e}")
...
def main():
    """
    Main entry point for LARGE_CINGULATE testing.
    """
    import argparse
    
    parser = argparse.ArgumentParser(description="Test LARGE_CINGULATE SAM-Med3D feature extraction and PCA pipeline")
    parser.add_argument('--data-path', type=str, default='crops/2mm/LARGE_CINGULATE.',
                       help='Path to LARGE_CINGULATE dataset')
    
    args = parser.parse_args()
    
    # Check if data path exists
    if not Path(args.data_path).exists():
        print(f"LARGE_CINGULATE data path not found: {args.data_path}")
        print("Please provide correct path to LARGE_CINGULATE dataset")
        sys.exit(1)
    
    # Check if config exists
    config_path = Path("sam3d/LARGE_CINGULATE/feature_extraction_lc.yaml")
    if not config_path.exists():
        print(f"Configuration file not found: {config_path}")
        print("Please ensure LARGE_CINGULATE configuration files are in place")
        sys.exit(1)
    
    # Run tests
    tester = LCFeatureExtractionPCATester(args.data_path)
